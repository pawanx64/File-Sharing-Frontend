...
export default function ForgotPassword() {
  ...
  const [step, setStep] = useState('email'); // 'email', 'otp', or 'newPassword'

  // Handles the email submission to request an OTP
  ...
  // Handles the OTP submission for verification
  const handleOtpSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsError(false);
        setStep('newPassword'); // Move to the final step
      } else {
  ...
  const InputField = ({ label, value, onChange, placeholder, showPassword, toggleVisibility, id, type = 'text', icon }) => (
    <div className="relative mb-5">
      ...
      <div className="relative">
        ...
        {icon && React.createElement(icon, {
          className: "w-5 h-5 absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-neutral-600"
        })}
        {toggleVisibility && (
          <button
            type="button"
